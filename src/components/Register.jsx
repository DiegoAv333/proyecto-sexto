...
export default function Register() {
    ...
    <section className={`${anim.fadeIn} px-4`} aria-labelledby="register-title">
        ...
        <div className="max-w-md w-full">
            ...
            <h1 id="register-title" className="text-3xl font-bold text-dark-gray">Registro</h1>
            </div>
            ...
            <form onSubmit={onSubmit} noValidate aria-describedby={err ? "register-error" : undefined}>
                ...
                <div className={form.field}>
                    ...
                    <input id="register-email" name="email" type="email" autoComplete="email" required className={form.input} placeholder="tu@email.com" />
                </div>
                <div className={form.field}>
                    <label htmlFor="register-password" className={form.label}>Contraseña</label>
                    <input id="register-password" name="password" type="password" autoComplete="new-password" required className={form.input} placeholder="••••••••" />
                </div>
                <div className={form.field}>
                    <label htmlFor="register-confirm" className={form.label}>Confirmar contraseña</label>
                    <input id="register-confirm" name="confirm" type="password" autoComplete="new-password" required className={form.input} placeholder="••••••••" />
                </div>
                </div>

                <div role="alert" id="register-error" aria-live="assertive" className={`${form.error} ${err ? "" : "hidden"}`}>
                {err}
                </div>

                <button type="submit" className="w-full mt-6 bg-strong-blue text-white py-3 px-6 rounded-xl font-medium hover:bg-blue-700">
                Registrarme
                </button>
            </form>
